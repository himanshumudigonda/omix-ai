            ...
            {mode !== AppMode.LIVE && (
              <div className="flex items-center gap-1 md:gap-2 bg-black/20 backdrop-blur-md p-1 rounded-full border border-white/10">
                {['auto', 'gemini', 'openai', 'meta', 'moonshot', 'qwen'].map((tab) => {
                  const tabLabels: Record<string, string> = { 'auto': 'A', 'gemini': 'G', 'openai': 'O', 'meta': 'M', 'moonshot': '🌙', 'qwen': 'Q' };
                  const tabColors: Record<string, string> = { 'auto': 'bg-gradient-to-r from-blue-500 to-purple-500', 'gemini': 'bg-blue-500', 'openai': 'bg-green-500', 'meta': 'bg-blue-600', 'moonshot': 'bg-yellow-500', 'qwen': 'bg-purple-500' };
                  return (
                    <button
                      key={tab}
                      onClick={() => {
